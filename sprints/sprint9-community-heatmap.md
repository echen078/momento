...
- **"My Photos" (default)** — shows the user's own photo pins on the map (Sprint 6 builds this). Users can click pins to see their photos, click the map to upload new ones, etc.
- **"Community Heatmap"** — hides the user's pins and instead shows a heatmap overlay of ALL public photos across the community. Hot zones (red/orange) = lots of photos. Cold zones (blue/transparent) = few or none. Includes time period filters: Past Week | Past Month | Past Year | All Time.
...
- Search for "react leaflet heatmap layer" for integration guides
...
- Query photos where `isPublic: true`, returning ONLY location and timestamp (use `.select('location createdAt')` to minimize payload)
